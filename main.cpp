...
public:
    ...
    void park(string reg_No,int age){
        if(noOfSlots==0){
            ...
            cout<<"Car with vehicle registration number"<<reg_No<<" has been parked at slot number "<<nearestAvailableSlot;
    ...
    void leave(int slot){
        if(noOfSlots==0){
          ...
          if(vehicleToLeave == NULL){
            ...
            vector<string> regNoList=mapAgeAndRegNo[vehicleToLeave->getDriverAge()];
            
      if(find(regNoList.begin(),regNoList.end(),vehicleToLeave->getRegistrationNumber())!=regNoList.end()){
              for(auto regNo=regNoList.begin();regNo!=regNoList.end();regNo++){
                if(*regNo==vehicleToLeave->getRegistrationNumber()){
                  regNoList.erase(regNo);break;
                }
              }
                           mapAgeAndRegNo[vehicleToLeave->getDriverAge()]=regNoList;
            }
            availableSlots.push_back(slot);
            cout<<"Slot number "<<slot<<" vacated, the car with vehicle registration number "<<vehicleToLeave->getRegistrationNumber()<< "left the space, the driver of the car was of age "<<vehicleToLeave->getDriverAge();
          }
     ...
     void getSlotNumberFromVehicleRegistrationPlate(string regi_No){
            if(noOfSlots==0){
                ...
                cout<<"The Vehicle with registration number "<<regi_No<<" not found in Parking Lot";
            }
    }
    ...
    void getSlotNumbersFromDriverAge(int age){
        ...
        else if(mapAgeAndRegNo.find(age)!=mapAgeAndRegNo.end() && mapAgeAndRegNo[age].size()>0){
            string slots="";
...
switch(input.size()){
            case 0:
            case 1:
            case 3:cout<<"invalid Command";
                    break;

            case 2  :if(input[0]=="Create_parking_lot")
            ...
            case 4:if(input[0]=="Park" && input[2]=="driver_age"&&input[1].length()==13){
